...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      startTransition(() => setError(err.message || 'Login failed'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-[#0a0908] flex items-center justify-center p-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-[#9a9387] text-sm">Sign in to your account</p>
        </div>
        ...
        <p className="text-center text-[#9a9387] text-sm">
          No account?{' '}
          <Link to="/signup" className="text-[#c5a572] hover:underline">Create one</Link>
        </p>
      </div>
    </div>
  )
