...
export default async function handler(req, res) {
  ...
  // Success — download from Replicate, upload to Supabase outputs, mark completed
  if (data.status === 'succeeded') {
    ...
    try {
      ...
      const ext = job.operation === 'bg_replace' ? 'webp' : 'webp'
      const outputPath = `${job.user_id}/${jobId}_result.${ext}`

      const { error: uploadError } = await supabase.storage
        .from('outputs')
        .upload(outputPath, buffer, { contentType: 'image/webp', upsert: true })
